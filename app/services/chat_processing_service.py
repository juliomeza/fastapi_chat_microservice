...
async def process_chat_message(db: AsyncSession, message: str, user_id: str) -> tuple[str, Optional[Any]]:
    ...
    is_for_data_orders = any(keyword in message.lower() for keyword in data_orders_keywords)
    ...
    if table == "data_datacardreport":
        ...
        rag_context = await get_rag_context(query=message, k=3, filter=rag_filter)
